<?php
    ...
    class branch extends db{
        ...
        $senderid,$emailaddress,$emailpassword,$smtpserver,$smtpport,$usessl,$invoicenoprefix,$invoicenocurrentno){
            if($this->checkbranch($branchid,'branchname',$branchname)){
                ...
                '{$senderid}','{$emailaddress}','{$emailpassword}','{$smtpserver}',{$smtpport},{$usessl},'{$invoicenoprefix}',{$invoicenocurrentno},{$this->userid},'{$this->platform}')";
                // echo $sql.PHP_EOL;
                $this->getData($sql);
                return "success";
            }
